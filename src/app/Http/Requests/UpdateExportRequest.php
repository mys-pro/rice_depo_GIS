        ...
        $validator->after(function ($validator) {
            ...
            foreach ($inputs as $key => $input) {
                ...
                if ($riceId != 0) {
                    ...
                    $totalExportDetailWeight = $this->exportRepository->getWeightDetail($id, $riceId);
                    $totalExportWeight = $this->exportRepository->getWeight($warehouseId, $riceId);

                    $availableWeight = $totalImportWeight - ($totalExportWeight - $totalExportDetailWeight);

                    if (($input['weight']) > $availableWeight) {
                        $validator->errors()->add("inputs.$key.weight", "Trọng lượng tồn kho không đủ. Hiện còn " . $availableWeight . "kg (" . ($key + 1) . ").");
